...
| `zoom-and-labels-test.mjs` | yes | Two OpenFrontIO-parity camera/label changes: scroll/pinch can now zoom out past `renderer.minScale` (the whole map exactly fills the viewport) down to a looser `renderer.minZoomScale` floor, so repeatedly zooming out the real way (`renderer.zoomAt`, the same call the wheel handler makes) clamps at that lower floor rather than the old one, leaving the map genuinely smaller than the viewport -- padded and centered on *both* axes, not pinned to an edge; and a name/troop label's rendered pixel size is no longer force-floored to a readable minimum, so the existing (previously dead, since the floor always sat above it) "too small, don't draw" cull can actually trigger. The label check holds camera position AND scale fixed and only varies the fake test player's `labelScale` between two calls, so the terrain pixels under the sampled region are identical in both cases and a real `renderer.ctx.getImageData` read of that region is a clean signal: near-white pixels present at a labelScale that computes above the cull threshold, none at one that computes below it, and hidden again after toggling back down -- confirming the cull runs live every frame rather than latching once. |
| `authored-maps-test.mjs` | no | Hand-authored maps -- World plus the six continents and four pattern maps added alongside it, all run through the identical checks in one parametrized loop rather than duplicated per map. Checks the things that would make an authored map unplayable rather than merely ugly: the grid is rectangular and uses only legend characters (and a ragged or mistyped one is rejected loudly rather than silently built into a broken world); dimensions, land share and terrain mix land where they were drawn; on every map that carries a river, every authored river cell is water at full resolution **and connects through to the open sea**, which is the load-bearing one -- rivers are ordinary water, so the only way across is by boat, and a river stranded in its own ocean component is a permanent wall no fleet can reach; every map (river or not) isn't fragmented into more than a handful of separate seas; `findSpawnPoints` fills every slot from its real plains-with-room path rather than the degenerate any-land fallback; terrain is byte-identical whatever the match seed is, while spawns still vary with it; and a real `Game` on the map spawns everyone, with river maps additionally checked for actually launching a landing force across a one-tile river channel. |
