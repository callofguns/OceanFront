...
| `difficulty-ui-verify.mjs` | yes | The difficulty picker renders Easy/Normal/Hard, persists the choice to `localStorage` and restores it on reload, and actually reaches `Game`/`AiController` (a bot's `aggression`, `goldMultiplier`, `troopsCapMultiplier` and `troopsMultiplier` all measurably increase across tiers with the same seed). Also checks that the attacking-troops HUD stat is hidden while idle, shows a live count during a real attack, and hides again once it settles. |
